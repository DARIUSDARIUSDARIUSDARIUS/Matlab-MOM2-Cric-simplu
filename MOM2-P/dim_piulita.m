function [Dbe,Dbi,Din,hg,Hp,Dg,Dg_prim,Dg_prim_min,De_prim,De,De_min] = dim_piulita(H,gamma,F,limita_rupere,D4,P)
if H > 200
    alpha = deg2rad(6);
else
    alpha = deg2rad(3);
...
De_min = round(sqrt( ((4 * gamma * F) / (pi * (limita_rupere/5))) + D4^2 ));   % limita rupere / 5 coeficient de siguranta
De = De_min + 7; % 7 mm pt siguranta intre 5 si 10
...
Hp = 10 * P; % inaltimea piulitei 10 fiind nr spire
